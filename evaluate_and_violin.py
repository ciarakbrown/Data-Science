...
def evaluate_sepsis_score(label_directory, prediction_directory):
    ...
    return auroc, auprc, accuracy, f_measure, normalized_observed_utility, observed_utilities, cohort_labels
...
if __name__ == '__main__':
    ...
    auroc, auprc, accuracy, f_measure, utility, observed_utilities, cohort_labels = evaluate_sepsis_score(
        LABEL_DIR,
        PRED_DIR
    )
...
# Assume observed_utilities and cohort_labels already exist

utilities_septic = [observed_utilities[i] for i in range(len(cohort_labels)) if np.any(cohort_labels[i])]
utilities_nonseptic = [observed_utilities[i] for i in range(len(cohort_labels)) if not np.any(cohort_labels[i])]

utility_df = pd.DataFrame({
    'Utility': utilities_septic + utilities_nonseptic,
    'Group': ['Septic'] * len(utilities_septic) + ['Non-Septic'] * len(utilities_nonseptic)
})

plt.figure(figsize=(8, 6))
sns.violinplot(x='Group', y='Utility', data=utility_df, palette='Blues', inner='box')
plt.title('Distribution of Utility Scores by Patient Group (Violin Plot) for LSTM')
...
metrics_df = pd.DataFrame({
    'Metric': ['AUC', 'F1', 'Utility'],
    'Score': [auroc, f_measure, utility]
})

plt.figure(figsize=(6, 5))
sns.barplot(x='Metric', y='Score', data=metrics_df, palette='Blues')
plt.title('LSTM Performance Comparison')
plt.ylabel('Score')
plt.ylim(0, 1)
plt.grid(True, axis='y')
plt.tight_layout()
